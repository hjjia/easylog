...
def saveRelation(data):
    ...
    if not cursor.fetchone():
        sql = "insert t_ajax_stage_relation(ajax_id,user_id,stage_id,cmd_form,create_time) values(%d,%d,%d,'%s',%d)" \
              %(data['ajax_id'], 0,data['stage_id'],data['cmd_form'],int(time.time()))
        cursor.execute(sql)
        db.commit()
...
def updateRelation(data):
    ...
    :return:
    """
    cmdForm = jsonify(data['cmd_form'])
    ajaxId = data['ajax_id']
    stageId = data['stage_id']
    sql = "update t_ajax_stage_relation set cmd_form = '%s' where ajax_id = %d and stage_id = %d "%(cmdForm,ajaxId,stageId)
    cursor.execute(sql)
    db.commit()
